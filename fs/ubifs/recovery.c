					 ...
					 int offs, void *sbuf, int grouped)
{
		...
		if (ret == SCANNED_EMPTY_SPACE) {
			if (!is_empty(buf, len)) {
				if (!is_last_write(c, buf, offs))
					break;
				printk(KERN_CRIT "%s: clean corrupt empty space (1)\n",
					   	__FUNCTION__);
	...
	if (!empty_chkd && !is_empty(buf, len)) {
		/*
		 * Patch from Lei Wan (Marvell):
		 * Regardless of whether is_last_write(), mark for cleaning
		 * instead of for corruption.
		 */
		clean_buf(c, &buf, lnum, &offs, &len);
		need_clean = 1;
		printk(KERN_CRIT "%s: clean corrupt empty space (2)\n", __FUNCTION__);
	}
