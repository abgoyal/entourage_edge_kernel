...
static void __init avengers_lite_init_mmc(void)
{
	...
	pdata = add_sd8x_rfkill_device(mfp_to_gpio(MFP_PIN_GPIO104), 0);
	if (pdata)
		avengers_lite_sdh3_platform_data.rfkill_pdata = pdata,
#endif
...
static gpio_u2h_oc = mfp_to_gpio(MFP_PIN_GPIO37);
...
static irqreturn_t avengers_lite_u2h_oc_handler()
{
	pr_debug("%s\n", __func__);
...
static void __init avengers_lite_add_nand(void)
{
	...
	avengers_lite_nand_info.use_dma = 0;
	avengers_lite_nand_info.RD_CNT_DEL = 0;
	avengers_lite_nand_info.enable_arbiter = 1;
	pxa168_add_nand(&avengers_lite_nand_info);
}

unsigned long mfp_read_boardid(unsigned long *cfgs, int num) {
	unsigned long id = 0;
	int i, s;
	...
	id >>= 1;/* shift back for last over shift */
	return id;
}

static void gpio_ec_init()
{
...
static void __init avengers_lite_init(void)
{
	...
	android_add_pmem("pmem_adsp", 0x00400000UL, 0, 1);
#endif
