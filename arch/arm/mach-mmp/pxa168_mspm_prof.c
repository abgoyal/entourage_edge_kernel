				...
				unsigned long val, void *data)
{
	 ...
	 * for pxa168, right now just return 0
	 */
	return 0;
}
		...
		const char *buf, size_t len)
{
	...
	if (data)
		...
		mspm_prof = DISABLE;
	/*
	 * If mspm_ctrl_state is disabled,
	 * we needn't enable/disable profiler here.
	 */
	if (mspm_ctrl_state == DISABLE)
		return len;
...
{
	struct op_info *info = NULL;
	struct pxa168_md_opt *md_op = NULL;
	int i, ret;
	...
	for (i = 0; i < mspm_op_num; i++) {
		...
		if (ret)
			continue;
		md_op = (struct pxa168_md_opt *)info->op;
