...
char i2c_bma020_write_buf(u8 dev_addr, u8 reg_addr, u8 *buf, u8 cnt)
{
	...
	return i2c_smbus_write_byte_data(g_client, reg_addr, *buf);
}
...
static int __devinit bma020_i2c_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
	...
	p_bma020->gpio = mfp_to_gpio(MFP_PIN_GPIO111);
#else
	p_bma020->gpio = mfp_to_gpio(MFP_PIN_GPIO36);
#endif
	g_client->irq = gpio_to_irq(p_bma020->gpio);
...
{
	#ifdef DEBUG_INFO
	printk("bma020 try attach to i2c bus.\n");
	#endif

	int ret;
	if ((ret = i2c_add_driver(&bma020_driver))) {
...
static ssize_t app_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
	...
	ret = bma020_read_reg(pos, local, read_len);
...
static int register_as_cdev(void)/* return 0 for success */
{
	...
	printk("register bma cdev, major: %d \n", p_bma020->app_major);
...
static int __init app_init(void)
{
	...
	if (!new_bma020) {
		printk(KERN_ERR "Failed to allocate bma020 device.\n");
		ret = -ENOMEM;
		goto ERROR0;
	}
	/* initialize global ptr */
