...
static struct wake_lock i2c_wakelock;
...
#ifdef DEBUG
	printk("i2c: <%s> slave_0x%x error: %s\n", i2c->adap.name, 
...
#if 0
	if (!i2c->use_pio)
		ret = request_irq(i2c->irq, i2c_pxa_handler, IRQF_DISABLED,
				  i2c->adap.name, i2c);
...
{
	wake_lock_init(&i2c_wakelock, WAKE_LOCK_SUSPEND, "i2c-pxa");
#ifdef CONFIG_DVFM
...
#endif
	wake_lock_destroy(&i2c_wakelock);
}
