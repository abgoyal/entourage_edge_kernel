					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		val->intval = di->BATT_CYCLE;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = (di->VOLT1 + di->VOLT2) * 4877 / 1000;
		break;

	case POWER_SUPPLY_PROP_CURRENT_NOW:
#ifndef CONFIG_ESI_EDGE_JR
		val->intval = di->CURRENT * 781 / 10000;     // 7.8125uA/LSB = 1.5625uV / 0.020 ohm
#else
		val->intval = di->CURRENT * 1562 / 10000;     // 15.625uA/LSB = 1.5625uV / 0.010 ohm
#endif
		break;
    ...
    case POWER_SUPPLY_PROP_STATUS: /*1--charging 2--discharging 4--Full 0--Unkhown 3--Not charging*/
        if ( di->STATUS == EC_NO_CONNECTION )
        ...
        else
          {
                ...
                else if(di->CURRENT > 0)
					val->intval = POWER_SUPPLY_STATUS_CHARGING;
				else
                    val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;

                if ( di->RARC > 100 )	// value of 255 is special trouble flag
                    val->intval = POWER_SUPPLY_STATUS_UNKNOWN;
				else if ( di->RARC >= 97 )
...
#else
	power_supply_changed(&di->bat);
	schedule_delayed_work(&di->monitor_work, HZ);
#endif
	return 0;
}
