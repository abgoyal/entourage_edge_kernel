 ...
 */
static int page_search(struct mtd_info *mtd, int start_page, int end_page,
 ...
 */
static int recover_new_bbm(struct mtd_info *mtd)
{
	struct pxa3xx_bbm *bbm = (struct pxa3xx_bbm *)mtd->bbm;
	struct pxa3xx_new_bbm *new_bbm = (struct pxa3xx_new_bbm *)bbm->data_buf;
	struct pxa3xx_bbt *fbbt = new_bbm->fbbt;
	...
	if (new_bbm->main_block == -1) {
		...
		for (boot_block = 0; boot_block < BOOT_PRAT_MAX; boot_block ++) {
			...
			/*
			 * Current NAND device needs ~2.4% reserved;
			 * so increase NEW_BBM_RELOC_PERCENTAGE by 24% using integer math
			 */
			reserved_block = reserved_block + (reserved_block * 24) / 100 ;
			printk(KERN_INFO "Reserve %d blocks for BBM RELOC\n",
					reserved_block);
#endif
