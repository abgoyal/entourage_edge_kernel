...
void gether_poll_controller(struct net_device *dev)
{
	if (!the_dev || !the_dev->gadget)
		return;

	usb_gadget_ioctl(the_dev->gadget, USB_GADGET_POLLING, 0);
}
...
int __init gether_setup(struct usb_gadget *g, u8 ethaddr[ETH_ALEN])
{
	...
	net->poll_controller = gether_poll_controller;
#endif
