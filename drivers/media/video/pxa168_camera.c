...
void ccic_set_clock_mipi(void)
{
	...
	//clk_enable(dbg_clk);	//enable MIPI by setting bit 25 of debug register 0xd4282888
	spin_lock_irqsave(&reg_lock, flags);
	__raw_writel(0x02000000 | __raw_readl(APMU_CCIC_DBG), APMU_CCIC_DBG);
...
static int __ccic_cam_internal_power_off(struct ccic_camera *cam)
{
	int zero = 0;
	return __ccic_cam_cmd(cam, VIDIOC_INT_INTERNAL_POWER_OFF, &zero);
}
...
static int ccic_cam_init(struct ccic_camera *cam)
{
	struct v4l2_chip_ident_old chip = { V4L2_CHIP_MATCH_I2C_ADDR, 0, 0, 0 };
	int ret;
	...
	if ((cam->sensor_type != V4L2_IDENT_OV7660) && (cam->sensor_type != V4L2_IDENT_OV3640)
							&& (cam->sensor_type != V4L2_IDENT_OV7740)) {
		...
		struct v4l2_buffer *buf)
{
	...
	flush_cache_range(sbuf->svma, sbuf->buffer, sbuf->buffer + cam->pix_format.sizeimage);
  out:
	mutex_unlock(&cam->s_mutex);
	return ret;
}
...
static int ccic_v4l_open(struct file *filp)
{
	...
	if (cam->users == 0) {
		if (cam->sensor) {
			...
			__ccic_cam_internal_power_off(cam);
		...
		struct v4l2_format *f)
{
	...
	f->fmt.pix.pixelformat = cam->pix_format.pixelformat;
	f->fmt.pix.width = cam->pix_format.width;
	f->fmt.pix.height = cam->pix_format.height;
	f->fmt.pix.field = V4L2_FIELD_NONE;
	mutex_lock(&cam->s_mutex);
	ret = __ccic_cam_cmd(cam, VIDIOC_G_FMT, &f->fmt);
	mutex_unlock(&cam->s_mutex);
...
static int ccic_v4l_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	{
		ret = ccic_vidioc_enum_framesizes(file, (void *)cam, arg);
		return ret;
	}
	default:
		break;
