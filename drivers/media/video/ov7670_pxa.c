		...
		struct ov7670_win_size **ret_wsize)
{
	...
	pix->bytesperline = pix->width*ov7670_formats[index].bpp/8;
...
static int ov7670_s_fmt(struct i2c_client *c, struct v4l2_format *fmt)
{
	...
	ov7670_write_array(c, ovfmt->regs);
...
static int ov7670_s_input(struct i2c_client *c, int *id)
{
	return 0;
}
...
static int __devinit ov7670_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int ret;
	...
	if (ret) {
		printk("%s: failed to detect ov7670!\n", __func__);
		goto out_free_info;
	}
	printk(KERN_NOTICE "OmniVision ov7670 sensor detected\n");
...
static int ov7670_g_register(struct i2c_client *client, struct v4l2_dbg_register * reg)
{
	return ov7670_read(client, (unsigned char)reg->reg, (unsigned char *)&(reg->val));
}

static int ov7670_s_register(struct i2c_client *client, struct v4l2_dbg_register * reg)
{
	return ov7670_write(client, (unsigned char)reg->reg, (unsigned char)reg->val);
}
#endif
		...
		void *arg)
{
	switch (cmd) {
		...
		case VIDIOC_S_INPUT:
			return ov7670_s_input(client, (int *) arg);
#ifdef CONFIG_VIDEO_ADV_DEBUG
		case VIDIOC_DBG_G_REGISTER:
			return ov7670_g_register(client, (struct v4l2_dbg_register *) arg);
		case VIDIOC_DBG_S_REGISTER:
			return ov7670_s_register(client, (struct v4l2_dbg_register *) arg);
#endif
	}
	return -EINVAL;
}
