...
static int ov7740_g_register(struct i2c_client *client, struct v4l2_dbg_register * reg)
{
	return ov7740_read(client, (unsigned char)reg->reg, (unsigned char *)&(reg->val));
}

//set ov register value directly, for debug usage
static int ov7740_s_register(struct i2c_client *client, struct v4l2_dbg_register * reg)
{
		...
		void *arg)
{
	...
	case VIDIOC_DBG_G_REGISTER:/*get register val for debug*/
		return ov7740_g_register(client, (struct v4l2_dbg_register *) arg);

	case VIDIOC_DBG_S_REGISTER:/*set register val for debug*/
		return ov7740_s_register(client, (struct v4l2_dbg_register *) arg);
#endif
	}
	return -EINVAL;
}
