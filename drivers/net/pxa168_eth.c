		...
		u8 *oaddr, u8 *addr)
{
	...
	printk(KERN_INFO "%s: ENTER, oaddr = %s addr = %s\n",
		__func__,
		(oaddr ? print_mac(mac, oaddr) : "NULL"),
		(addr ? print_mac(mac1, addr) : "NULL"));
#endif
...
static void pxa168_eth_set_rx_mode(struct net_device *dev)
{
	...
	for (i = 0; i < dev->mc_count; i++) {
		...
		printk(KERN_INFO "%s: %d : %s\n",
			__func__,
			i, print_mac(mac, mclist->dmi_addr));
#endif
...
#if 0
	printk(KERN_INFO "%s: ENTER, oldMac = %s dev->dev_addr = %s\n",
		__func__,
		((oldMac[0] | oldMac[1] | oldMac[2]) ?
			print_mac(mac, oldMac) : "NULL"),
		(dev->dev_addr ?
			print_mac(mac1, dev->dev_addr) : "NULL"));
#endif
...
static irqreturn_t pxa168_eth_int_handler(int irq, void *dev_id)
{
...
#if 0
		printk(KERN_INFO "Link ISR: mii_link_ok = %d netif_carrier_ok = %d\n",
			mii_link_ok(&mp->mii),netif_carrier_ok(dev));

		printk(KERN_INFO "mp->tx_desc_count = %d\n",
				mp->tx_desc_count);
#endif
	...
	if (icr & (ICR_RXBUF | ICR_RXERR | ICR_TXBUF_H |
		...
		if (netif_rx_schedule_prep(&mp->napi)) {
			/* Disable interrupts */
			wrl(mp, INT_MASK, 0);
			__netif_rx_schedule(&mp->napi);
		}
	}
#else
...
#if 0
	printk(KERN_INFO "%s: mii_link_ok = %d netif_carrier_ok = %d\n",
		__func__,
		mii_link_ok(&mp->mii), netif_carrier_ok(dev));
#endif
 ...
 */
static void eth_mac_addr_set(char *str, unsigned char *device_mac)
{
	unsigned int mac[6];
	...
	device_mac[0] = (unsigned char)mac[0];
	device_mac[1] = (unsigned char)mac[1];
	device_mac[2] = (unsigned char)mac[2];
	device_mac[3] = (unsigned char)mac[3];
	device_mac[4] = (unsigned char)mac[4];
	device_mac[5] = (unsigned char)mac[5];
}
#endif
...
#if 0
	/* not now -- can cause chip to hang */
	if (!mii_link_ok(&mp->mii) && netif_carrier_ok(dev)) {
		/* link now down */
		netif_stop_queue(dev);
		netif_carrier_off(dev);
...
static int pxa168_eth_probe(struct platform_device *pdev)
{
	...
	dev->open = pxa168_eth_open;
	dev->stop = pxa168_eth_stop;
	dev->hard_start_xmit = pxa168_eth_start_xmit;
	dev->set_mac_address = pxa168_eth_set_mac_address;
	dev->set_multicast_list = pxa168_eth_set_rx_mode;
	/* No need to Tx Timeout */
	dev->tx_timeout = pxa168_eth_tx_timeout;

	dev->watchdog_timeo = 2 * HZ;
	dev->base_addr = 0;
	dev->change_mtu = pxa168_eth_change_mtu;
	dev->do_ioctl = pxa168_eth_do_ioctl;
