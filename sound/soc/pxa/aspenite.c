...
static int aspenite_wm8753_hifi_shutdown(struct snd_pcm_substream *substream)
{
	dbg("aspenite_wm8753_hifi_shutdown\n");
	return 0;
}

static int aspenite_wm8753_hifi_hw_params(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->dai->codec_dai;
	return 0;
	return snd_soc_dai_set_sysclk(codec_dai, WM8753_MCLK , 12000000,
			SND_SOC_CLOCK_IN);
}
...
static int aspenite_wm8753_voice_shutdown(struct snd_pcm_substream *substream)
{
...
static int __init aspenite_init(void)
{
	...
	struct wm8753_setup_data *setup = (struct wm8753_setup_data *)&wm8753_setup;
